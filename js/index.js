...
let xScale = d3.scaleTime()
          .domain([new Date(2016, 0, 1), new Date()]) // TODO: find min/max dates from data timestamps
          .range([0, 700]); // 700px wide
...
d3.select('.timeline').on('mousemove', function() {
  ...
  var active = d3.select('.camera-item.active');
  if (active.size() === 0) {
